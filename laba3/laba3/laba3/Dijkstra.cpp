...
void Dijkstra(int matrix[9][9], int startPoint = 0)
{
	...
	pointValue[startPoint] = 0;

	for (int j = 0; j < 8; j++)
	{
		minPointValue = INT32_MAX;

		for (int i = 0; i < 9; i++) // минимальное значение узла из всех и его индекс
		{
		...
		checkedNode[minPointIndex] = true; 
	...
	for (int i = 0; i < 9; i++) {
		if (i != startPoint) {
			cout << "\nпуть от " << Convert(startPoint + 1) << " до " << Convert(i + 1) << " = " << pointValue[i];
		}
	}
}
